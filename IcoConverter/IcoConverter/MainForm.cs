        ...
        void ConvertToIco(Image img, string file)
        {
            using (var msIco = new MemoryStream())
            {
                using (var bw = new BinaryWriter(msIco))
                {
                    ...
                    bw.Write((short)vs.Length); //Specifies number of images in the file.

                    List<byte[]> images = new List<byte[]>();

                    int currentPos = (16 * vs.Length) + 6;

                    foreach (int current in vs)
                    {
                        ...
                        {
                            new Bitmap(img, current == 0 ? 256 : current, current == 0 ? 256 : current).Save(_ms, ImageFormat.Png);
                            _ms.Position = 0;
                            _vs = new byte[_ms.Length];
                            _ = _ms.Read(_vs, 0, (int)_ms.Length);
                            images.Add(_vs);
                        }
                        bw.Write(_vs.Length);
                        bw.Write(currentPos);
